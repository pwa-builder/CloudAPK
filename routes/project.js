...
const analytics_1 = require("../build/analytics");
...
router.post(['/generateAppPackage', '/generateApkZip'], async function (request, response) {
    ...
    const analyticsInfo = {
        url: ((_a = apkRequest.options) === null || _a === void 0 ? void 0 : _a.pwaUrl) || ((_b = apkRequest.options) === null || _b === void 0 ? void 0 : _b.host) || '',
        packageId: ((_c = apkRequest.options) === null || _c === void 0 ? void 0 : _c.packageId) || '',
        name: ((_d = apkRequest.options) === null || _d === void 0 ? void 0 : _d.name) || '',
    ...
    if (apkRequest.validationErrors.length > 0 || !apkRequest.options) {
        ...
        urlLogger_1.logUrlResult(((_e = apkRequest.options) === null || _e === void 0 ? void 0 : _e.pwaUrl) || ((_f = apkRequest.options) === null || _f === void 0 ? void 0 : _f.host) || '', false, errorMessage);
        analytics_1.trackEvent(analyticsInfo, errorMessage, false);
    ...
    try {
        ...
        analytics_1.trackEvent(analyticsInfo, null, true);
        ...
        analytics_1.trackEvent(analyticsInfo, errorString, false);
        response
            .status(500)
...
router.get("/fetch", async function (request, response) {
    ...
    if (!fetchResult.ok) {
        response
            ...
            .send(`Unable to fetch ${url}. Status: ${fetchResult.status}, ${fetchResult.statusText}`);
        return;
    }
    if (fetchResult.type) {
    ...
    try {
        ...
        response
            .status(500)
            .send(`Unable to fetch result from ${url} using type ${type}. Error: ${getResultError}`);
    }
});
function validateApkRequest(request) {
    ...
    validationErrors.push(...requiredFields.filter((f) => !options[f]).map((f) => `${f} is required`));
...
async function createAppPackageWith403Fallback(options, projectDirPath, signing) {
    ...
    try {
        const bubbleWrapper = new bubbleWrapper_1.BubbleWrapper(options, projectDirPath, signing, http1Fetch);
        ...
        if (is403Error) {
            ...
            const bubbleWrapper = new bubbleWrapper_1.BubbleWrapper(optionsWithSafeUrl, projectDirPath, signing, http2Fetch);
            return await bubbleWrapper.generateAppPackage();
        }
